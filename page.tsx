...
interface AnalyseScores {
  update_me: number
  educate_me: number
  give_me_perspective: number
  divert_me: number
  inspire_me: number
  help_me: number
  connect_me: number
  keep_me_engaged: number
}
...
interface AnalyseResultWithChart extends AnalyseResult {
...
function Analyser({ sessionToken, onUse }: { sessionToken: string; onUse: (remaining: number) => void }) {
  ...
  const [results, setResults] = useState<AnalyseResultWithChart | null>(null)
  ...
  const analyzeText = async () => {
    ...
    try {
      ...
      const parsed = JSON.parse(raw) as AnalyseResult

      const chartData: ChartDatum[] = USER_NEEDS.map(need => {
        const key = (need.id === 'update' ? 'update_me' : need.id === 'educate' ? 'educate_me' :
          ...
          need.id === 'connect' ? 'connect_me' : 'keep_me_engaged') as keyof AnalyseScores
        return { name: need.name, score: parsed.scores?.[key] ?? 0, color: need.color }
      })
      ...
      {results && (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8">
            ...
            {results.secondary_recommendations && results.secondary_recommendations.length > 0 && (
              <>
...
function StoryIdeation({ sessionToken, onUse }: { sessionToken: string; onUse: (remaining: number) => void }) {
  ...
  const [ideas, setIdeas] = useState<IdeateResult | null>(null)
  const [error, setError] = useState<string | null>(null)
  ...
  const generateIdeas = async () => {
    ...
    try {
      ...
      const raw = data.content[0].text.trim().replace(/```json\n?/g, '').replace(/```\n?/g, '')
      setIdeas(JSON.parse(raw) as IdeateResult)
    } catch (err) {
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-6">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900">User Needs Tool</h1>
